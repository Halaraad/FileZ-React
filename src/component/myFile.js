...
componentDidMount(FolderIdCheck) {
  ...
  filterFiles(value){
    ...
    if (value == 'Name') {
      ...
      let sort = this.state.Files;
      const result = sort.filter(sort => sort.type != 'image' && sort.type != 'application/pdf' && sort.type != 'video');
      this.setState({
        Files: result
      })
    ...
    return (
      ...
      >
          {this.state.Folders.map(Folder => (
                ...
                content={
                  <Menu>
                     <Menu.Group title="All Files ">
                      <Menu.Item icon="projects" onClick={()=>{
                        this.filterFiles('all')
                      }}>all Files</Menu.Item>
                    </Menu.Group>
                    <Menu.Divider />
                    <Menu.Group title="Sort By ">
                      <Menu.Item icon="sort-alphabetical" onClick={()=>{
                        this.filterFiles('Name')
                      }}>Name</Menu.Item>
                      <Menu.Item icon="sort-asc" onClick={()=>{
                        this.filterFiles('Size')
                      }}>Size</Menu.Item>
                    </Menu.Group>
                    <Menu.Divider />
                    <Menu.Group title="Filter File by">
                      <Menu.Item icon="media" onClick={()=>{
                        this.filterFiles('images')
                      }}>
                      images files 
                      </Menu.Item>
                      <Menu.Item icon="print" onClick={()=>{
                        this.filterFiles('PDF')
                      }}>
                      PDF Files
                      </Menu.Item>
                      <Menu.Item icon="video" onClick={()=>{
                        this.filterFiles('Videos')
                      }}>
                      Videos Files
                      </Menu.Item>
                      <Menu.Item icon="paperclip" onClick={()=>{
              ...
              >
                <Button marginRight={16} iconAfter="caret-down">Files</Button>
              </Popover>

              <IconButton id="undoFolder"  icon="undo" intent="success" onClick={()=>{
                                if (document.getElementById("Openfolder")) {
                                  document.getElementById("Openfolder").src =`${FolderIcon}`
                                  document.getElementById("Openfolder").id = iconbuffer;
                ...
                <Table>
                  <Table.Head>
                    <Table.TextCell
                      ...
                      flexGrow={0}
                    />
                    <Table.TextHeaderCell marginLeft={20}>Name</Table.TextHeaderCell>
                    <Table.TextHeaderCell marginLeft={20}>Uploaded At</Table.TextHeaderCell>
                  ...
                  <Table.Body height={538}>
                    {this.state.Files.map(file => (
                      ...
                      >
                        ...
                        <Table.TextCell marginLeft={40}>{file.uptime}</Table.TextCell>
                        <Table.TextCell marginRight={-40}>{(file.size/1000000).toFixed(3)} MB</Table.TextCell>
                        ...
                        <Table.TextCell
                          ...
                          flexGrow={0}
                        >
                          <IconButton icon="import" intent="success" onClick={()=>{
                            window.open(
